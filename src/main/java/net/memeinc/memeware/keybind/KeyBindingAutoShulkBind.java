...
package net.memeinc.memeware.keybind;
...
import net.memeinc.memeware.procedure.ProcedureAutoShulkBindOnKeyPressed;
...
public class KeyBindingAutoShulkBind extends ElementsMemewareMod.ModElement {
	private KeyBinding keys;
	public KeyBindingAutoShulkBind(ElementsMemewareMod instance) {
		super(instance, 57);
	}
	...
	public void init(FMLInitializationEvent event) {
		keys = new KeyBinding("key.mcreator.auto_shulk_bind", Keyboard.KEY_0, "key.categories.misc");
	...
	public void onKeyInput(InputEvent.KeyInputEvent event) {
		if (Minecraft.getMinecraft().currentScreen == null) {
			if (org.lwjgl.input.Keyboard.isKeyDown(keys.getKeyCode())) {
				MemewareMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage());
	...
	private static void pressAction(EntityPlayer entity) {
			...
			return;
		{
			...
			ProcedureAutoShulkBindOnKeyPressed.executeProcedure($_dependencies);
		}
	}
}
